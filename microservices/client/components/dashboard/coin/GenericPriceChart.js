...
class GenericPriceChart extends SocketComponent {
  constructor(props) {
  ...
  onSocketData = (data) => {
    this.setState({ ...this.state, pricesList: data, previousValue: data[data.length-2] });
  }
  render() {
    const { pricesList = [], previousValue = 0 } = this.state;
    const priceIncreased = pricesList.length ? Math.abs(pricesList[pricesList.length - 1] - previousValue) > 0 : false;
